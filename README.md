...
1. Explain the difference between Relational Databases and SQL.
   A Relational Database is where data is stored in a table that is grouped into rows and columns (like a spreadsheet)
   SQL stands for Structured Query Language and is used to write relational databased.
2. Why do tables need a Primary Key?
   Tables need a primary key because they identify data on a specific row on a table.
3. What is the name given to a table column that references the Primary Key on another table?
   A Foreign key is used to identify the primary key of another table.
4. What do we need in order to have a _many to many_ relationship between two tables?
   To have a many to many relationship between tables we need an intermediary table that holds foreign keys that reference the primary keys on the related tables.
